...
import {
  ...
  assertSafeText
} from '../validators/contentSafety.js';
import {
  awardXp,
...
const TEACHER_UUID_PATTERN =
  /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;

function generateTeacherUuid() {
  return crypto.randomUUID();
}

async function findTeacherByIdentifier(identifier, options = {}) {
  const value = String(identifier || '').trim();

  if (TEACHER_UUID_PATTERN.test(value)) {
  ...
  if (/^[1-9]\d*$/.test(value)) {
    return Teacher.findByPk(Number(value), options);
  }
...
function assignedLessonWhere(assignments) {
  ...
  where.gradeLevel = [
    ...new Set(
      assignments.map(
        (assignment) => Number(assignment.gradeLevel)
      )
    )
  ];
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const duplicateAccount = await findDuplicateAccount({
        accountType: 'teacher',
        name: body.name,
        email: body.email,
      ...
      if (duplicateAccount) {
        return res.status(409).json(
          duplicateAccountPayload(duplicateAccount)
        );
      }
      ...
      if (err?.name === 'SequelizeUniqueConstraintError') {
        ...
        const messages = {
          ...
          uuid: 'Teacher UUID already exists. Please retry.'
        };
  ...
  async (req, res, next) => {
    try {
      ...
      if (assignedStudentIds) {
        completedWhere.studentId = assignedStudentIds;
      }
      ...
      return res.json({
        stats: {
          ...
          classProgress:
            lessons && students
              ? Math.round(
                  (completed / (lessons * students)) * 100
                )
              : 0
        },
        recentStudents,
  ...
  async (req, res, next) => {
    try {
      ...
      for (const attempt of attempts) {
        ...
        if (Number(row.attemptNo) === 1) {
          item.attempt1 = attemptData;
        }

        if (Number(row.attemptNo) === 2) {
          item.attempt2 = attemptData;
        }

        item.bestPercent = Math.max(
          item.bestPercent,
          Number(row.percent || 0)
        );

        item.latestPercent = Number(
          row.percent ||
          item.latestPercent ||
          0
        );
      ...
      const rows = Array.from(grouped.values()).sort(
        (a, b) => {
          if (a.gradeLevel !== b.gradeLevel) {
            return (
              Number(a.gradeLevel || 0) -
              Number(b.gradeLevel || 0)
            );
          }
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const completed = await CompletedLesson.findAll({
        where: {
          studentId: student.id
        },
        include: [Lesson]
...
router.patch(
  ...
  async (req, res, next) => {
    try {
      const teacher = await findTeacherByIdentifier(
        req.params.teacherIdentifier,
        {
          include: [safeUserInclude()],
        }
      );
      ...
      if (req.body.name && teacher.User) {
        teacher.User.displayName = req.body.name;
...
router.post(
  '/:teacherIdentifier/reset-password',
  requireRole('admin'),
  async (req, res, next) => {
    try {
      const teacher = await findTeacherByIdentifier(
      ...
      if (
        ...
        return res.status(422).json({
          message:
            'Reactivate the teacher account before resetting the password.'
        });
      }
      ...
      const temporaryPin =
        req.body.temporaryPin?.trim() ||
        generateTemporaryPin();

      if (!/^[2-9]{4}$/.test(temporaryPin)) {
        return res.status(422).json({
          message:
            'Temporary PIN must be exactly 4 digits using numbers 2-9.'
        });
      }

      teacher.User.passwordHash = await bcrypt.hash(
        temporaryPin,
        12
      );
...
function canTeacherReviewWriting(
  student,
  lesson,
  row = {}
) {
  const status = normalizeWritingReviewStatus(
    row.reviewStatus
  );
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const writing =
        writingRows.map(normalizeRow);

      const speech =
        speechRows.map(normalizeRow);
      ...
      const writingTaskIds = uniqueIds(
        writing.map(
          (row) => row.taskId
        )
      );

      const speechTaskIds = uniqueIds(
        speech.map(
          (row) => row.taskId
        )
      );
      ...
      const writingTaskById =
        byId(writingTasks);

      const speechTaskById =
        byId(speechTasks);
      ...
      const writingItems = writing.map(
        (row) => {
          ...
          const task = writingTaskById.get(
            Number(row.taskId)
          );

          const reviewStatus =
            normalizeWritingReviewStatus(
              row.reviewStatus
            );

          const reviewEligible =
            canTeacherReviewWriting(
              student,
              lesson,
              row
            );
          ...
          return {
            ...
            reviewedByTeacherId:
              row.reviewedByTeacherId ||
              null,
            reviewEligible,
            xpPreview:
              reviewStatus === 'graded' &&
              score !== null
                ? writingScoreToXp(score)
                : null,
      ...
      const speechItems = speech.map(
        (row) => {
          const task = speechTaskById.get(
            Number(row.taskId)
          );
          ...
          return {
            ...
            feedback:
              row.feedback || '',
            reviewStatus:
              normalizeSpeechReviewStatus(row),
            reviewedAt:
...
router.patch(
  ...
  async (req, res, next) => {
    try {
      ...
      const submission =
        await WritingSubmission.findByPk(
          submissionId
        );
      ...
      if (!student || !lesson) {
        return res.status(404).json({
          message:
            'Writing submission is missing student or lesson details.'
        });
      }
      ...
      if (
        ...
        return res.status(403).json({
          message:
            'You can only grade writing submissions from your assigned students.'
        });
      }
      ...
      if (
        ...
        return res.status(422).json({
          message:
            'Only Grade 3 to Grade 6 writing submissions can be graded by teachers.'
        });
      }
      ...
      if (currentStatus !== 'pending') {
        return res.status(409).json({
          message:
            'This writing submission has already been graded.',
          submission: {
            id: submission.id,
            ...
            reviewedAt:
              ...
              null,
            reviewedByTeacherId:
              submission.reviewedByTeacherId ||
              null,
          },
          xpAwarded: 0,
        });
      ...
      const [updatedCount] =
        await WritingSubmission.update(
          {
            reviewStatus: 'graded',
            score,
            feedback:
      ...
      if (!updatedCount) {
        ...
        return res.status(409).json({
          message:
            'This writing submission was already graded. Please refresh the page.',
          submission: latest
            ? {
                id: latest.id,
                reviewStatus:
      ...
      return res.json({
        submission: {
          ...
          reviewedAt:
            ...
            null,
          reviewedByTeacherId:
            updatedSubmission.reviewedByTeacherId ||
            null,
          student: {
            id: student.id,
            name: student.name,
        ...
        message:
          `Writing grade saved. Student earned +${xpAwarded} XP.`,
      });
    } catch (err) {
      return next(err);
    }
  ...
  async (req, res, next) => {
    try {
      ...
      if (!feedback && !hasScore) {
        return res.status(422).json({
          message:
            'Add feedback or a score before saving the speech review.'
        });
      }
      ...
      if (!student || !lesson) {
        return res.status(404).json({
          message:
            'Speech attempt is missing student or lesson details.'
        });
      }

      const assignedStudentIds =
        await speechReviewAssignedStudentIds(
          req
        );
      ...
      if (
        ...
        return res.status(403).json({
          message:
            'You can only review speech attempts from your assigned students.'
        });
      }
