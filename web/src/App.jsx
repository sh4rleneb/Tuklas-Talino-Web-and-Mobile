...
if (role === 'admin') {
  ...
  async function submitQuizzes(quiz, answers) {
    ...
    if (existingAttempts.length >= maxQuizAttempts) {
      ...
      notify('Nagamit mo na ang 2 pagsubok sa pagsusulit. Balikan na lang ang iyong mga sagot.');
      go('screen-stu-quiz-result');
    saveStudentNavigationState({
      screen: 'screen-stu-quiz-result',
      selectedQuizzesId: quiz?.id || selectedQuizzes?.id || null,
    ...
    api(`/lessons/${lessonId}/quiz-result`, {
      ...
      .then((data) => {
        ...
        if (syncedResult.xpAwarded) {
          ...
          notify(`Naipasa ang pagsusulit: ${syncedResult.score}/${syncedResult.total} (${syncedResult.percent}%). Naisave ang ulit na pagsubok, walang dagdag na XP.`);
        } else {
    ...
    if (activity?.type === 'mcq') {
      asArray(activity.questions).forEach((question, questionIndex) => {
        ...
        questions.push({
          ...
          type: 'mcq',
          source: activity.title || 'Quizzes sa Lessons',
...
function buildStudentQuizzes(data = {}) {
  return asArray(data?.lessons).map((lesson) => {
    ...
    return {
      ...
      title: `Quizzes sa ${lesson.title || 'Lessons'}`,
      ...
      type: lesson.completed ? 'Quizzes Pagkatapos ng Lessons' : 'Pagsasanay na Quizzes',
      unlocked: true,
      questions
    };
...
function Grade46StudentDashboard({ data, openLesson, openFirstSubjectLesson, goStudentTab, logout, selectedAvatar }) {
  ...
  const planCards = asArray(data?.lessons)
    ...
    .slice(0, 4)
    .map((lesson, index) => {
      const subject = formatStudentSubjectDisplay(lesson.subject || 'Lessons');
      return {
        lesson,
        ...
        <div className="g46-ref-column">
          ...
          <section className="g46-ref-panel">
            <div className="g46-ref-panel-head">
              ...
              <button type="button" className="g46-ref-panel-link" onClick={() => goStudentTab('groups')}>Buksan ang grupo →</button>
            </div>
            ...
            {groupTask ? (
              ...
              <div className="g46-ref-empty">Wala pang gawaing panggrupo. Magaling! 🎉</div>
            )}
          </section>
        </div>
  ...
  return (
    ...
    >
      <section className="g12-section-card">
        ...
        {!lessons.length && (
          <div className="g12-empty">Wala pang aralin para sa filter na ito.</div>
        )}
      </section>
    </EarlyStudentChrome>
  );
...
function LessonsScreen({ lessons, subjectFilter, setSubjectFilter, go, openLesson, data, logout}) {
  ...
  return (
    <Grade46StudentChrome
      ...
      icon="📚"
      title="Lessons"
      subtitle="Pumili ng aralin o modyul sa Filipino para magpatuloy."
    >
      <section className="g46-ref-panel">
        <div className="g46-ref-panel-head">
          <div>
        ...
        {!lessons.length && <div className="g46-ref-empty">Wala pang aralin para sa filter na ito.</div>}
      </section>
    </Grade46StudentChrome>
  );
}
  ...
  const lessonSteps = [
    {
      ...
      subtitle: 'Tingnan ang layunin ng lesson, gantimpala, at kailangang tapusin.'
    },
    ...(materialActivities.length ? [{
      key: 'material',
      eyebrow: 'Step 2',
      title: 'Materyal ng Lessons',
    ...
    },
    {
      ...
      subtitle: activityTotal
        ...
        : 'Wala pang gawaing pagsasanay para sa lesson na ito.'
    },
    {
      key: 'complete',
      eyebrow: `Hakbang ${materialActivities.length ? 5 : 4}`,
      title: 'Tapusin ang Lessons',
      subtitle: 'Ipasa ang pag-unlad sa lesson kapag handa ka na.'
    }
  ];
  ...
  function renderCompletedBuod() {
    return (
      <Grade46StudentChrome
        ...
        logout={logout}
        icon="✅"
        title={lesson?.title || "Lesson Buod"}
        ...
        <div className="g46-ref-stack" style={{ display: 'grid', gap: 16 }}>
          ...
          >
            <div className="g46-ref-panel-head">
              <div>
                ...
                <h2 style={{ marginTop: 8 }}>Lesson Buod</h2>
          ...
          {materialActivities.length > 0 && (
            <section className="g46-ref-panel">
              <div className="g46-ref-panel-head">
                <div>
                  <h2>Balikan ang Materyal ng Lessons</h2>
...
function getEarlyLessonTopicTitle(lesson = {}) {
  ...
  if (!rawTitle) return compactKidTitle(subject || "Lessons");
  ...
  if (!topic || /^[\d\s-–—:]+$/.test(topic)) {
    topic = subject || withoutExtra || "Lessons";
  }
...
function makeStudentFriendlyPassage(lesson) {
  ...
  const extracted = extractSectionFromLessonPlan(
    ...
    ['Vocabulary Words', 'Maikling Quizzes', 'Gawain sa Pagtutugma', 'Gawain sa Pagsulat', 'Pagsasanay sa Pagbigkas', 'Teacher Notes']
  );
...
.g12-reward-actions {
      ...
      <div className="g12-mission-wrap">
        {rewardModal && (
          <div className="g12-reward-overlay" role="dialog" aria-modal="true" aria-label="Gantimpala sa Missions">
        ...
        <section className="g12-mission-banner">
          ...
          <div className="g12-mission-path" aria-label="Mga Hakbang sa Missions">
            ...
            <>
              <div className="g12-finish-card">
                <div>
                  ...
                  <h3>{isReviewMode ? 'Tapos na ang Pagbabalik-aral!' : 'Tapos na ang Missions!'}</h3>
                  <p>
                    {isReviewMode
                      ? 'Natapos mo na ang araling ito. Maaari kang bumalik sa Tahanan o pumili ng ibang aralin.'
  ...
  return (
    <div className="card" style={activityBoxStyle}>
      <div className="row" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
        <div className="section-title">
          {isEarlyGrade ? '📎 Slides ng Lessons' : 'Materyal ng Lessons'}
        </div>
      ...
      >
        ...
        <span className="muted">
          {isPdf
            ...
            : 'Buksan muna ang lesson slides bago sagutin ang mga gawain.'}
        </span>
        ...
        {fileUrl ? (
          ...
          >
            {isPdf ? 'Buksan ang Buong PDF' : 'Buksan ang Slides ng Lessons'}
          </a>
        ) : (
...
function McqActivity({ activity, index, total, isEarlyGrade, activityBoxStyle, submitMcq }) {
  ...
  return (
    <div className="card" style={activityBoxStyle}>
      <div className="row" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
        <div className="section-title">
          {isEarlyGrade ? '🎮 Maikling Quizzes' : 'Quizzes'}
        </div>
      ...
      <div className="muted">
        {submitted
          ? isEarlyGrade
            ? 'Tapos na! Naisave na ang iyong sagot.'
    ...
    <div className="card" style={activityBoxStyle}>
      ...
      <div className="row" style={{ gap: 12, marginTop: 16, flexWrap: 'wrap' }}>
        <button className="btn btn-purple" onClick={speakTarget} style={{ minHeight: isEarlyGrade ? 56 : 48, fontSize: isEarlyGrade ? 18 : 16, padding: '12px 24px', borderRadius: 999 }}>
          {isEarlyGrade ? '🔊 Pakinggan' : '🔊 Makinig'}
        </button>
        ...
        <button className="btn btn-outline" onClick={() => stopSpeech()} style={{ minHeight: isEarlyGrade ? 56 : 48, fontSize: isEarlyGrade ? 18 : 16, padding: '12px 24px', borderRadius: 999 }}>
          {isEarlyGrade ? '⏹ Ihinto' : '⏹ Ihinto ang Audio'}
        </button>
      </div>
...
function StudentMissions({ data, go, onPlayMission, logout}) {
  ...
  const availableCount = games.filter(game => game.status !== 'Naka-lock').length;
  ...
  const playMission = (game) => {
    if (!game || game.status === 'Naka-lock') return;
      ...
      <div className={`missions-wrap ${early ? 'early' : 'standard'}`}>
        <section className="missions-hero">
          <div className="missions-hero-copy">
            ...
            <h2>{early ? 'Missions ng Tuklas' : 'Mga Larong Pang-aral'}</h2>
            <p>
              {early
        ...
        <section className="missions-section">
          ...
          <div className="missions-game-grid">
            {games.map(game => {
              const locked = game.status === 'Naka-lock';
...
function StudentMissionPlay({ data, go, selectedGameId = 'word-match', onBack, refresh, logout}) {
  ...
  const locked = selectedGame?.status === 'Naka-lock';
        ...
        <section className="mission-play-shell">
          <div className="mission-play-card">
            ...
            <div className="mission-play-head">
              ...
              <div>
                <h2>{selectedGame?.title || 'Missions'}</h2>
                <p>
                  {locked
            ...
            {locked ? (
              <>
                <div className="mission-prompt-box">
                  🔒 Naka-lock na misyon. Tapusin ang {selectedGame?.minCompleted || 3} aralin o misyon para mabuksan ang hamong ito.
                </div>

                <div className="mission-play-actions">
                  <button type="button" className="mission-play-action secondary" onClick={backToMissions}>
                    ← Bumalik sa mga Missions
                  </button>
                </div>
              </>
            ) : (
                ...
                {isWordMatch ? (
                  ...
                  <div className={`sentence-builder-game ${early ? 'early' : 'standard'} ${sentenceBuilderWrong ? 'wrong' : ''} ${sentenceBuilderCorrect ? 'correct' : ''}`}>
                    <div className="sentence-builder-prompt">
                      <div className="sentence-builder-mini-label">Missions sa Pangungusap</div>
...
<button type="button" className="mission-play-action" onClick={restartDemo}>
                  ...
                  {isWordMatch ? (
                    <button type="button" className="mission-play-action purple" onClick={completeWordMatchMission} disabled={!wordMatchComplete || missionSaving}>
                      {missionSaving ? 'Saving...' : '✅ Tapusin ang Missions'}
                    </button>
        ...
        <div className="g12-page">
          <header className="g12-topbar">
            <button type="button" className="g12-brand" onClick={backToMissions}>
              ...
              <span>{selectedGame?.title || 'Missions'}</span>
            </button>
  ...
  return (
    <Grade46StudentChrome
      ...
      title={selectedGame?.title || 'Missions'}
                          ...
                          <div className="g46-submit-grid">
                            ...
                            <div className="g46-submit-field">
                              ...
                              <label className="g46-upload-drop" htmlFor={`group-file-${task.id}`}>
                                ...
                                <small>{submittedFileName ? 'Napili na ang file at handa nang ipasa' : 'PDF, image, document, or screenshot'}</small>
                              </label>
                            </div>
                          </div>
...
function badgeDisplayDescription(badge = {}) {
  ...
  if (code === 'group_1') return 'Natapos ang 1 aprubadong gawaing panggrupo.';
...
function badgeAchievementReason(badge = {}) {
  ...
  if (normalizedCode === 'group_1' || value.includes('teamwork') || value.includes('group') || value.includes('pangkat') || value.includes('kasama') || value.includes('kaagapay')) {
    return 'Natapos ang 1 aprubadong gawaing panggrupo.';
  }
...
const GRADE12_BADGE_GOALS = [
  ...
  },
  {
    ...
    howToUnlock: 'Tapusin ang 1 aprubadong gawaing panggrupo.'
  },
  {
    code: 'xp_100',
    icon: '⭐',
      ...
      <section className="g12-section-card">
        <div className="g12-section-head">
          <div>
            ...
            <p className="g12-section-subtitle">Pangunahing impormasyon sa profile at progreso.</p>
          </div>
        </div>
    ...
    >
      ...
      <section className="g46-ref-panel">
        <div className="g46-ref-panel-head">
          <div>
            <h2>Buod</h2>
            <p className="g46-ref-muted">Pangunahing impormasyon sa profile at progreso.</p>
          </div>
        </div>
